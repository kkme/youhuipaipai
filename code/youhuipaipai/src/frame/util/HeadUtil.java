...
public class HeadUtil {
	...
	public HeadUtil(Context context){
		...
		if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
			createFile(Constant.IMG_URL);
		}else {
			ToastUtil.showLong(context, "亲,没有内存卡,无法上传头像了");
		}
	}
	public void takePhoto(){//照相
		Intent intentPaizhao = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
		/*intentPaizhao.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(Environment.getExternalStorageDirectory(), "/imageName.jpg")));*/
		intentPaizhao.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(Constant.IMG_URL,"/imagename.jpg")));
		((Activity)context).startActivityForResult(intentPaizhao, PHOTOHRAPH);
	}
	public void takePhoto1(){//照相
		Intent intentPaizhao = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
		/*intentPaizhao.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(Environment.getExternalStorageDirectory(), "/imageName.jpg")));*/
		intentPaizhao.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(Constant.IMG_URL,"/personimage.jpg")));
		((Activity)context).startActivityForResult(intentPaizhao, PHOTOHRAPH);
	}
