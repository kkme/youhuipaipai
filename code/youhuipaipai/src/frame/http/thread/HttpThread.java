...
public abstract class HttpThread extends FrameThread{
	...
	/**
	 * 网络请求
	 * @param httpRequestBean  请求参数
	 * @param callBack		   回调函数
	 * @param callBackID     回调函数ID
	 */
	public HttpThread(HttpRequestBean httpRequestBean,IHttpCallBack callBack,int callBackID){
	 ...
	 * @param callBack		   回调函数
	 * @param callBackID     回调函数ID
	 * @param isTest	            是否启用测试回调
	 */
	public HttpThread(HttpRequestBean httpRequestBean,IHttpCallBack callBack,int callBackID,boolean isTest){
	...
	public static HttpThread getHttpThread(Class threadClass){
		if(threadClass==null)return null;
		Object obj=null;
		try {
