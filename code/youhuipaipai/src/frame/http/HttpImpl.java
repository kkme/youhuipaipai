...
//				return new JSONObject(value);
	 ...
	 * <br>JSONObject json=new JSONObject();
		<br> try {
		<br>	json.put("name", "灰灰11");
		<br>	json.put("pass", "123456");
		<br>   } catch (JSONException e) {
		<br>	e.printStackTrace();
		<br>}
		<br>String signature=ImgUtil.md5("tSzaVWlQFS_1r7XG1v94NitwDXSHDL360qTHorHQZ-E"+json.toString());
		<br>http.doPostJSON("http://www.quyeba.com/api/v2/mobile/user_login.json"+"?signature="+signature, json);
	 * @param uri  请求地址
	 ...
	 *	}<br>
	 *	String signature=ImgUtil.md5("tSzaVWlQFS_1r7XG1v94NitwDXSHDL360qTHorHQZ-E"+json.toString());<br>
	 * 	http.doPostJSON("http://www.quyeba.com/api/v2/mobile/user_login.json"+"?signature="+signature, json,"application/json");<br>
	 * @param uri 请求地址
	 * @param json 请求参数json对象
	 * @param contentType  http请求头类型 例如 application/json
	 * @return
	 */
	public HttpResultBean doPostJSON(String uri,JSONObject json,String contentType){
		return doPostString(uri, json.toString(),contentType);
	}

	/**
	 * HTTP的Post请求 列如：
		<br>http.doPostJSON("http://www.quyeba.com/api/v2/mobile/user_login.json","{\"pass\":\"123456\",\"name\":\"灰灰11\"}");
	 * @param uri  请求地址
	 * @param body 请求参数string 类型 
	 * @return
	 */
	public HttpResultBean doPostString(String uri,String body){
		return doPostString(uri,body,"application/json");
	}
	
	/**
	 * HTTP的Post请求 列如：
		<br>http.doPostJSON("http://www.quyeba.com/api/v2/mobile/user_login.json","{\"pass\":\"123456\",\"name\":\"灰灰11\"}","application/json");
	 * @param uri  请求地址
	 * @param body 请求参数string 类型 
	 * @param contentType  http请求头类型 例如 application/json
	 * @return
	 */
	public HttpResultBean doPostString(String uri,String body,String contentType){
		try {
			return doPostByte(uri,body.getBytes("utf-8"),"application/json");
	...
	public HttpResultBean doPostByte(String uri, byte[] body,String contentType){
		...
		try {
			RequestEntity requestEntity = new ByteArrayRequestEntity(body,contentType);
...
//				return new JSONObject(value);
	 ...
	 * HTTP的Post请求   表单方式提交
	 * @param uri  请求地址  
	 * @param map  表单键值对  例如  map.put("nickname", "zhangsan5");
	 * @return
	 */
	 ...
	 * @param uri  请求地址  
	 * @param map  表单键值对  例如  map.put("filename", new File(""));
	 * @return
	 */
	 ...
	 * @param uri  请求地址  
	 * @param map  表单键值对  例如  map.put("filename", new File(""));
	* @param contentType  http请求头类型 例如 application/json  application/octet-stream
	 * @return
	 */
	public HttpResultBean doPostMapWithFile(String uri,Map map,String contentType){
	...
	* @param contentType  http请求头类型 例如 application/json  application/octet-stream
	 * @return
	 */
	public HttpResultBean doPutMapWithFile(String uri,Map map,String contentType){
		return doMethodMapWithFile("put", uri, map, contentType);
	}
	
	
	private HttpResultBean doMethodMapWithFile(String mflag,String uri,Map map,String contentType){
		...
		try {
			...
			for (Object key : map.keySet()) {
				...
				if(obj.getClass()==File.class){
					MyFilePart filePart=new MyFilePart(str, (File)obj,contentType);
					...
					UTF8StringPart stringPart=new UTF8StringPart(str, obj.toString());
	...
	class MyFilePart extends FilePart{
		...
		public MyFilePart(String name, File file,String contentType) throws FileNotFoundException {
			super(name, file);
